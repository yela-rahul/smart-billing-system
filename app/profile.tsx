...
export default function Profile() {
  ...
  const saveProfile = async () => {
    try {
      const uid = auth.currentUser?.uid;
      if (!uid) return;
      await updateDoc(doc(db, "users", uid), { fullName, shopName, phone });
      setUser((prev: any) => ({ ...prev, fullName, shopName, phone }));
  ...
  const changePassword = async () => {
    try {
      ...
      if (!oldPwd || !newPwd) { Alert.alert("Error", "Please enter both old and new password."); return; }
    ...
    Alert.alert("Logout", "Are you sure you want to logout?", [
      { text: "Cancel" },
      {
        ...
        onPress: async () => {
          try {
            const uid = auth.currentUser?.uid;
            if (uid) { await updateDoc(doc(db, "users", uid), { lastLogout: new Date().toISOString() }); }
            await signOut(auth);
            await clearAuth();
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.heroCard}>
          <View style={styles.heroRow}>
            ...
            <View style={styles.heroTextContainer}>
              ...
              <Text style={styles.heroName} numberOfLines={1}>{user?.fullName || "User Name"}</Text>
        ...
        <View style={styles.gridContainer}>
          ...
          <View style={styles.gridCard}>
            ...
            <Text style={styles.gridValue} numberOfLines={1}>{user?.shopName || "N/A"}</Text>
        ...
        {/* ✅ PRINTER SETTINGS — NEW ITEM ADDED */}
        <TouchableOpacity
        ...
        >
          <View style={styles.listLeft}>
            ...
            <View>
              <Text style={styles.listTitle}>Printer Settings</Text>
              <Text style={styles.listSub}>Connect Bluetooth printer</Text>
            </View>
          </View>
          <Ionicons name="chevron-forward" size={20} color="#CBD5E1" />
        </TouchableOpacity>

        <TouchableOpacity style={styles.listItem} onPress={() => setPwdModal(true)}>
          <View style={styles.listLeft}>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            ...
            <View style={styles.inputGroup}>
              ...
              <TextInput style={styles.textInput} value={fullName} onChangeText={setFullName} placeholderTextColor="#94A3B8" />
            </View>
            <View style={styles.inputGroup}>
            ...
            <View style={styles.modalActions}>
              <TouchableOpacity onPress={() => setEditVisible(false)} style={styles.btnCancel}>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            ...
            <View style={styles.inputGroup}>
              ...
              <TextInput style={styles.textInput} value={oldPwd} onChangeText={setOldPwd} secureTextEntry placeholderTextColor="#94A3B8" />
            </View>
            <View style={styles.inputGroup}>
              <Text style={styles.inputLabel}>New Password</Text>
              <TextInput style={styles.textInput} value={newPwd} onChangeText={setNewPwd} secureTextEntry placeholderTextColor="#94A3B8" />
            </View>
            <View style={styles.modalActions}>
              <TouchableOpacity onPress={() => setPwdModal(false)} style={styles.btnCancel}>
                <Text style={styles.btnCancelText}>Cancel</Text>
              </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  heroCard: {
    backgroundColor: "#2563EB", borderRadius: 24, padding: 24, marginBottom: 24,
  ...
  heroWelcome: { color: "#BFDBFE", fontSize: 14, fontWeight: "500" },
  heroName: { color: "#FFFFFF", fontSize: 22, fontWeight: "bold", marginBottom: 2 },
  heroShop: { color: "#E0E7FF", fontSize: 14, opacity: 0.9 },
  editProfileBtn: {
  ...
  editProfileText: { color: "#FFFFFF", fontWeight: "600", marginRight: 6, fontSize: 14 },
  ...
  gridValue: { fontSize: 16, fontWeight: "700", color: "#0F172A", marginBottom: 2 },
  gridLabel: { fontSize: 13, color: "#64748B" },
  ...
  versionText: { color: "#64748B", fontWeight: "600" },
  listMeta: { color: "#0F172A", fontWeight: "600", fontSize: 14 },

  logoutButton: {
    flexDirection: "row", alignItems: "center", justifyContent: "center",
    marginTop: 12, backgroundColor: "#FEF2F2", paddingVertical: 16,
  ...
  modalContainer: { backgroundColor: "#FFFFFF", borderRadius: 24, padding: 24 },
  modalHeader: { fontSize: 20, fontWeight: "700", color: "#0F172A", marginBottom: 20, textAlign: "center" },
  ...
  btnCancel: { flex: 1, backgroundColor: "#F1F5F9", padding: 14, borderRadius: 12, alignItems: "center" },
  btnCancelText: { color: "#64748B", fontWeight: "600", fontSize: 15 },
  btnSave: { flex: 1, backgroundColor: "#2563EB", padding: 14, borderRadius: 12, alignItems: "center" },
